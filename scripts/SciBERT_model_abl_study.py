...
def train(max_length, epochs, feature_subset, train_df, test_df, val_df):
    ...
    fixed_feature = ['cite_context_sent_+2']
    target_column = 'citation_class_label'
    features = fixed_feature + list(feature_subset)
    print(features)
    ...
    val_df[features] = val_df[features].astype(str)


    # Combine all the textual features into a single feature
    train_df['combined_text'] = train_df[features].apply(' [SEP] '.join, axis=1)
    test_df['combined_text'] = test_df[features].apply(' [SEP] '.join, axis=1)
    val_df['combined_text'] = val_df[features].apply(' [SEP] '.join, axis=1)
...
def main():
    ...
    # Fixed feature
    fixed_feature = ['cite_context_sent_+2']
    #, 'self_citation', 'direct_citations', 'cited_publication_date', 'cited_publication_info', 'sent_pos_in_article', 'cited_abstract'
    # Features for ablation study
    features = ['cite_pos_in_sent', 'self_citation', 'direct_citations', 'cited_publication_date', 'cited_publication_info', 'sent_pos_in_article', 'cited_abstract']
